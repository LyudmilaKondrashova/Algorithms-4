...
public class Tree {
    ...
    public boolean isLeaf(Node current) { // true, если данный узел дерева является листом
        return current != null && current.left == null && current.right == null;
    }

    public int getChildrenCount() {
        return getChildrenCount(root, 0);
    }

    public int getChildrenCount(Node current, int count)
    {
        if (current == null) {
            return count;
        }
        ...
        count = getChildrenCount(current.left, count) +
                getChildrenCount(current.right, count);
        return count;
    }
    // Конец задания 2 ******************
